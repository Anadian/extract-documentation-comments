...
function Log_Error( error ){
	console.error('Log_Error received: %o', error);
}
...
class JsdocToMarkdown {
  ...
  render (options) {
	  var _return = null;
	  console.log('render received: %o',options);
    options = options || {}
    const dmdOptions = new DmdOptions(options)
	  console.log('dmdOptions: %o', dmdOptions);
    if (options.data) {
      _return = dmd.async(options.data, dmdOptions).catch(Log_Error);
    } else {
      _return = this.getTemplateData(options)
        .then(templateData => dmd.async(templateData, dmdOptions)).catch(Log_Error);
    }
	  console.log('render returned: %o', _return);
	  return _return;
  }
  ...
  getTemplateData (options) {
	  console.log('getTemplateData received: %o', options);
    options = options || {}
    const jsdocParse = require('jsdoc-parse')
	  console.log('jsdocParse: %o', jsdocParse);
    return this.getJsdocData(options)
      .then(jsdocParse).catch(Log_Error);
  }
  ...
  getJsdocData (options) {
	  console.log('getJsdocData received: %o', options);
    const jsdocOptions = new JsdocOptions(options)
	  console.log('jsdocOptions: %o', jsdocOptions);
  ...
  clear () {
    ...
    return jsdocApi.cache.clear().then(() => dmd.cache.clear()).catch(Log_Error);
  }
  ...
  getNamepaths (options) {
	  console.log('getNamepaths: %o', options);
